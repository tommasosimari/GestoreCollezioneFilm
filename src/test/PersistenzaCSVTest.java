...
class PersistenzaCSVTest {

    //Questo è un file di test; non quello che useremo alla fine
    private String file1 = "collezione_test.csv";
    private File file2 = new File("C:\\Users\\simar\\IdeaProjects\\GestoreCollezioneFilm\\src\\test\\test.csv");

    private PersistenzaIF persistenza;

    @BeforeEach
    void setUp() throws Exception {
        persistenza = new PersistenzaCSV(file1);
    ...
    void testCaricaESalva() throws Exception {
        PersistenzaCSV persistenza = new PersistenzaCSV(file1);
        List<Film> listaCaricataCSV = persistenza.carica();
        assertEquals(2, listaCaricataCSV.size());
        ...
        List<Film> test_salvataggio = new ArrayList<>(listaCaricataCSV);
